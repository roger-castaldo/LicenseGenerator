...
namespace Org.Reddragonit.LicenseGenerator
{
	...
	internal class JSON
    {
        ...
        private static readonly Regex _regDateTime = new Regex("\\d{4}-((0[1-9])|(1[0-2]))-((0[1-9])|([12]\\d)|(3[01]))T([0-5]\\d):([0-5]\\d):([0-5]\\d).\\d{3}Z", RegexOptions.Compiled | RegexOptions.ECMAScript);
        ...
        private static readonly Regex _regDate = new Regex("^\\d{4}-\\d{2}-\\d{2}$", RegexOptions.Compiled | RegexOptions.ECMAScript);
        private const string _DateFormat = "yyyy-MM-dd'T'HH:mm:ss.fff'Z'";
        ...
        protected bool SerializeValue(object value, StringBuilder builder)
        {
            ...
            {
                SerializeString(((DateTime)value).ToString(_DateFormat), builder);
            }
